...
def run(
    ...
    while idx < len(combined_paths):
        ...
        # Served API schemas are endpoint-specific evidence: parse live
        # OpenAPI/Swagger documents into modeled operations (dead-code fix:
        # schemas were fetched but never ingested).
        if clean_path.lower().endswith(("openapi.json", "swagger.json", "swagger.yaml",
                                        "openapi.yaml", "openapi.yml", "swagger.yml")) \
                and resp.status_code == 200:
            try:
                _doc = resp.json()
                if isinstance(_doc, dict) and isinstance(_doc.get("paths"), dict):
                    openapi_docs.append(_doc)
            except Exception:
                pass
    ...
    try:
        ...
        for _doc in openapi_docs:
            try:
                ingest_openapi_document(app, _doc, source="openapi")
            except Exception:
                continue
